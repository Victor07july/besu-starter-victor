...
CORREÇÃO: Usa 'pending' nonce para evitar conflitos
"""
...
def invoke_smart_contract(w3, contract, account, vehicle_data):
    ...
    while retry_count < max_retries:
        try:
            # 🔧 CORREÇÃO: Usar 'pending' para incluir transações não confirmadas
            nonce = w3.eth.get_transaction_count(account.address, 'pending')
            
            print(f"Usando nonce: {nonce} (tentativa {retry_count + 1}/{max_retries})")
            ...
            # Verificar se é erro de nonce
            if 'replacement transaction underpriced' in error_msg.lower() or \
               'nonce too low' in error_msg.lower():
                retry_count += 1
                if retry_count < max_retries:
                    print(f"Conflito de nonce detectado. Retentando... ({retry_count}/{max_retries})")
                    import time
                    time.sleep(1)  # Aguardar 1s antes de retry
                    continue
            
            # Erro não relacionado a nonce
            return {
                'success': False,
                'error': error_msg
            }
    ...
    return {
        'success': False,
        'error': f'Failed after {max_retries} retries due to nonce conflicts'
    }
...
def lambda_handler(event, context):
    ...
    try:
        ...
        for idx, record in enumerate(records):
            ...
            # Extrair atributos da mensagem
            message_group_id = record.get('attributes', {}).get('MessageGroupId', 'unknown')
            ...
            try:
                ...
                # Invocar contrato (com retry automático para nonce)
                ...
                print(f"METRIC|{wallet}|exception|{duration:.3f}|0|failed")
        ...
        total_e1 = sum(r.get('e1_value', 0) for r in results if r.get('success'))
        total_gas = sum(r.get('gas_used', 0) for r in results if r.get('success'))
        ...
        print(f"❌ Erro fatal no handler: {str(e)}")
        return {
            'statusCode': 500,
            'body': json.dumps({
                'error': str(e)
