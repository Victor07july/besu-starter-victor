...
def main():
    """Função principal"""
    if len(sys.argv) < 2:
        print("Uso: python3 visualize_trips.py <trips_trajectories.json> [output.html] [vehicle_id]")
        print("\nExemplos:")
        print("  python3 visualize_trips.py trips_sumo_processed_trajectories.json")
        print("  python3 visualize_trips.py trips_sumo_processed_trajectories.json mapa.html")
        print("  python3 visualize_trips.py trips_sumo_processed_trajectories.json mapa.html SUMO_carro_1000")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'trip_comparison.html'
    vehicle_filter = sys.argv[3] if len(sys.argv) > 3 else None
    ...
    # Criar mapa
    create_trip_map(trajectories, None, output_file, vehicle_filter)
    
    # Instruções
    print("\n💡 COMO USAR:")
    print(f"   1. Abra o arquivo: {output_file}")
    print("   2. Use o controle de camadas (canto superior direito) para:")
    print("      - Mostrar/ocultar trajetos originais")
    print("      - Mostrar/ocultar trajetos com DP")
    print("      - Mostrar/ocultar linhas de deslocamento")
    print("      - Mostrar/ocultar pontos intermediários")
    print("   3. Clique nos marcadores e linhas para ver detalhes")
    print("   4. Use a ferramenta de medição para medir distâncias")
    print("\n💡 Cada linha mostra o TRAJETO COMPLETO (não apenas início→fim)!")
