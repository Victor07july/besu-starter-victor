...
try:
    ...
    print("✓ Biblioteca SRTM carregada")
except ImportError:
    ELEVATION_LIB = None
    ELEVATION_DATA = None
    print("⚠️  SRTM não instalado. Elevação não será calculada.")
    print("    Instalar: pip install srtm.py ou executar ./elevation/install.sh")
...
class DifferentialPrivacyGPS:
    ...
    def __init__(self, epsilon: float = EPSILON, search_radius: int = SEARCH_RADIUS):
        ...
        self.graphs_cache = {}
        self.elevation_cache = {}  # Cache de elevações
    ...
    def get_elevation(self, lat: float, lon: float) -> int:
        """
        Obtém elevação de uma coordenada
        ...
        # Método 1: SRTM (Offline - Rápido)
        if ELEVATION_LIB == "srtm" and ELEVATION_DATA is not None:
            try:
                elev = ELEVATION_DATA.get_elevation(lat, lon)
                if elev is not None:
                    elevation = int(elev)
            except Exception as e:
                print(f"⚠️  Erro SRTM: {e}")
    ...
    def process_coordinates(self, lat_orig: float, lon_orig: float) -> Dict:
        """
        Pipeline completo de processamento com privacidade diferencial + ELEVAÇÃO
        ...
        # ETAPA 1.5: Obter elevação ORIGINAL (antes do DP)
        ...
        # ETAPA 3.5: Obter elevação PRIVADA (depois do DP)
        ...
        return {
            ...
            'elevation_original': elevation_original,  # Elevação original
            'elevation_private': elevation_private    # Elevação privada
        }
    ...
    def process_trip(self, start_coord: str, end_coord: str) -> Dict:
        ...
        print(f"   Elevação início (original):  {start_result['elevation_original']}m")
        print(f"   Elevação início (privada):   {start_result['elevation_private']}m")
        print(f"   Elevação fim (original):     {end_result['elevation_original']}m")
        print(f"   Elevação fim (privada):      {end_result['elevation_private']}m")
