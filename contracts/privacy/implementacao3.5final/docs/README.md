...
**Saída:**
- Arquivo `dados_..._private.csv` contendo:
  - ✅ Todas as colunas originais (preservadas sem modificação)
  - ✅ **10 novas colunas** com coordenadas protegidas e métricas
  - 📊 Veja seção [Colunas do CSV de Saída](#-colunas-do-csv-de-saída) para detalhes
...
O script de privacidade diferencial adiciona as seguintes colunas ao CSV original:

### 🔐 Coordenadas Protegidas

| Coluna | Tipo | Descrição |
|--------|------|-----------|
| `start_lat_private` | float | **Latitude protegida do ponto inicial**. Coordenada após aplicação de ruído Laplaciano e map matching para via trafegável. Esta é a coordenada que deve ser enviada para blockchain. |
| `start_lon_private` | float | **Longitude protegida do ponto inicial**. Coordenada após aplicação de ruído Laplaciano e map matching para via trafegável. |
| `end_lat_private` | float | **Latitude protegida do ponto final**. Coordenada após aplicação de ruído Laplaciano e map matching para via trafegável. |
| `end_lon_private` | float | **Longitude protegida do ponto final**. Coordenada após aplicação de ruído Laplaciano e map matching para via trafegável. |

### 📊 Métricas de Privacidade e Qualidade

| Coluna | Tipo | Descrição |
|--------|------|-----------|
| `start_displacement_m` | float | **Deslocamento do ponto inicial em metros**. Distância euclidiana entre a coordenada original (`start_location`) e a coordenada protegida (`start_lat_private`, `start_lon_private`). Indica o nível de privacidade aplicado. Valores típicos: 50-500m dependendo do epsilon. |
| `end_displacement_m` | float | **Deslocamento do ponto final em metros**. Distância euclidiana entre a coordenada original (`end_location`) e a coordenada protegida (`end_lat_private`, `end_lon_private`). |
| `gps_distance_private_km` | float | **Distância da viagem calculada com coordenadas protegidas**. Calculada usando fórmula de Haversine entre os pontos protegidos. Este valor substitui a distância original para preservar privacidade. |

### 🛠️ Metadados do Processamento

| Coluna | Tipo | Descrição |
|--------|------|-----------|
| `dp_epsilon` | float | **Valor de epsilon (ε) usado no processamento**. Parâmetro de privacidade diferencial configurado na execução. Valores menores = mais privacidade (mais ruído). Exemplo: 0.5 |
| `dp_processed` | boolean | **Indica se a linha foi processada com sucesso**. `True` = processamento bem-sucedido, `False` = ocorreu erro. |
| `dp_error` | string | **Mensagem de erro** (apenas se `dp_processed = False`). Descreve o problema que impediu o processamento da linha. |

### 💡 Interpretação dos Valores

**Deslocamento (displacement):**
- **< 100m**: Privacidade baixa, alta utilidade
- **100-300m**: Privacidade moderada, boa utilidade
- **300-500m**: Privacidade alta, utilidade moderada
- **> 500m**: Privacidade muito alta, pode reduzir utilidade dos dados

**Exemplo de linha processada:**

```csv
VIN,start_location,end_location,start_lat_private,start_lon_private,end_lat_private,end_lon_private,start_displacement_m,end_displacement_m,gps_distance_private_km,dp_epsilon,dp_processed
93XATGK1WSCR19187,"-5.843199, -35.197724","-5.843128, -35.197571",-5.843456,-35.197889,-5.843301,-35.197423,187.3,142.8,5.921,0.5,True
```

**Observações importantes:**
- ⚠️ As colunas originais (`start_location`, `end_location`) **não são modificadas**
- ✅ Use sempre as colunas `*_private` para envio ao blockchain
- ✅ As coordenadas protegidas estão **garantidamente em vias trafegáveis**
- ✅ O deslocamento permite auditar o nível de privacidade aplicado
...
### Exemplo 3: Análise de Privacidade e Qualidade dos Dados
...
# Gráfico 1: Distribuição de deslocamento (início)
axes[0, 0].hist(df['start_displacement_m'], bins=30, color='steelblue', edgecolor='black')
axes[0, 0].set_xlabel('Deslocamento (metros)')
axes[0, 0].set_ylabel('Frequência')
axes[0, 0].set_title(f'Deslocamento Inicial (ε={df["dp_epsilon"].iloc[0]})')
axes[0, 0].axvline(df['start_displacement_m'].mean(), color='red', 
                    linestyle='--', label=f'Média: {df["start_displacement_m"].mean():.1f}m')
axes[0, 0].legend()

# Gráfico 2: Distribuição de deslocamento (fim)
axes[0, 1].hist(df['end_displacement_m'], bins=30, color='coral', edgecolor='black')
axes[0, 1].set_xlabel('Deslocamento (metros)')
axes[0, 1].set_ylabel('Frequência')
axes[0, 1].set_title(f'Deslocamento Final (ε={df["dp_epsilon"].iloc[0]})')
axes[0, 1].axvline(df['end_displacement_m'].mean(), color='red', 
                    linestyle='--', label=f'Média: {df["end_displacement_m"].mean():.1f}m')
axes[0, 1].legend()

# Gráfico 3: Boxplot comparativo
axes[1, 0].boxplot([df['start_displacement_m'], df['end_displacement_m']], 
                    labels=['Início', 'Fim'])
axes[1, 0].set_ylabel('Deslocamento (metros)')
axes[1, 0].set_title('Comparação de Deslocamentos')
axes[1, 0].grid(axis='y', alpha=0.3)

# Gráfico 4: Distâncias das viagens
axes[1, 1].hist(df['gps_distance_private_km'], bins=30, color='green', 
                edgecolor='black', alpha=0.7)
axes[1, 1].set_xlabel('Distância (km)')
axes[1, 1].set_ylabel('Frequência')
axes[1, 1].set_title('Distribuição de Distâncias (Protegidas)')

plt.tight_layout()
plt.savefig('analise_privacidade.png', dpi=300, bbox_inches='tight')
print("📊 Gráficos salvos em: analise_privacidade.png")
plt.show()
