...
def process_sumo_csv(input_csv: str, consumo_fabricante: float = CONSUMO_FABRICANTE, row_step: int = ROW_STEP, max_radius_km: float = MAX_RADIUS_KM) -> pd.DataFrame:
    """
    ...
    # Preserva a ordem original do CSV para desempate quando timestamps forem iguais.
    ...
    for vehicle_id, group in df.groupby('vehicle_id'):
        ...
        # Ordenar de forma estavel para evitar zig-zag quando muitos start_time sao iguais.
        sort_cols = [c for c in ['start_time', 'end_time'] if c in group.columns]
        sort_cols.append('_row_order')
        group = group.sort_values(sort_cols, kind='mergesort')
        ...
        # Distancias SUMO sao acumuladas; usar maximo e mais robusto que ultimo quando a ordem varia.
        total_distance_km = pd.to_numeric(group['distance'], errors='coerce').max() if len(group) > 0 else 0
        distance_city_km = pd.to_numeric(group['distance_city'], errors='coerce').max() if len(group) > 0 else 0
        distance_highway_km = pd.to_numeric(group['distance_highway'], errors='coerce').max() if len(group) > 0 else 0

        total_distance_km = float(total_distance_km) if pd.notna(total_distance_km) else 0.0
        distance_city_km = float(distance_city_km) if pd.notna(distance_city_km) else 0.0
        distance_highway_km = float(distance_highway_km) if pd.notna(distance_highway_km) else 0.0
...
def main():
    """Função principal"""
    if len(sys.argv) < 2:
        print("Uso: python3 process_sumo_csv.py <input.csv> [output.csv] [consumo_fabricante] [row_step] [max_radius_km] [num_runs]")
        print("\nExemplos:")
    ...
    for run_id in range(1, num_runs + 1):
        ...
        df_trips, trajectories = process_sumo_csv(input_file, consumo_fab, step, max_radius)
