...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nO script irá gerar um HTML para cada veículo em uma subpasta maps ao lado do JSON")
        sys.exit(1)
    ...
    # Criar diretório de saída se não existir (subpasta maps no diretório do JSON)
    ...
    for idx, traj in enumerate(trajectories, start=1):
        ...
        print(f"\n🚗 Gerando mapa para {vehicle_id} (trajeto/run {run_id})...")
