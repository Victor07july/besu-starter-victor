#!/usr/bin/env python3
"""
Script para processar dados SUMO com offset ALEATÓRIO (veh0)

=== VERSÃO HAVERSINE PURA + OFFSET ALEATÓRIO ===
...
- Processa apenas o trajeto do veh0
- Offset aleatório dentro de um raio máximo (passado via CLI)
  - Ângulo aleatório uniforme em [0°, 360°)
  - Distância aleatória uniforme no disco (sqrt para distribuição uniforme)
...
# Offset Aleatório
MAX_RADIUS_KM = 2.0  # Raio máximo de deslocamento (km) - pode ser sobrescrito via CLI
...
TARGET_VEHICLE_ID = None    # None: auto-detect (veh0 se existir, senão primeiro)
...
def apply_deterministic_offset(lat: float, lon: float, offset_x: float, offset_y: float, max_radius_km: float = MAX_RADIUS_KM) -> tuple:
    """
...
def generate_random_offset(max_radius_km: float, ref_lat: float = 0.0) -> tuple:
    """
    Gera offset aleatório (offset_x, offset_y) dentro do raio máximo.
    Distribuição uniforme no disco (sqrt para evitar concentração no centro).

    Args:
        max_radius_km: Raio máximo do deslocamento em km
        ref_lat: Latitude de referência para conversão longitude (padrão: equador)

    Returns:
        Tupla (offset_x_degrees, offset_y_degrees, distance_km, angle_degrees)
    """
    # Ângulo aleatório uniforme em [0, 2π)
    angle = np.random.uniform(0, 2 * np.pi)

    # Distância aleatória uniforme no disco
    distance_km = np.sqrt(np.random.uniform(0, 1)) * max_radius_km

    # Componentes cartesianas em km
    dx_km = distance_km * np.cos(angle)  # contribuição latitude
    dy_km = distance_km * np.sin(angle)  # contribuição longitude

    # Converter para graus
    offset_x = dx_km / 111.32
    cos_lat = np.cos(np.radians(ref_lat))
    offset_y = dy_km / (111.32 * cos_lat) if cos_lat != 0 else 0.0

    return offset_x, offset_y, distance_km, np.degrees(angle)
...
def _sanitize_id_component(value: str) -> str:
    """Normaliza componentes de ID para uso seguro em nomes de arquivo."""
    text = re.sub(r'[^A-Za-z0-9_-]+', '-', str(value).strip())
    text = re.sub(r'-{2,}', '-', text).strip('-')
    return text if text else "unknown"


def build_trajectory_id(source_csv: str, run_id: int, vin: str, traj_seq: int) -> str:
    """Gera ID único do trajeto rastreável por CSV e execução."""
    source_stem = os.path.splitext(os.path.basename(source_csv))[0]
    return (
        f"{_sanitize_id_component(source_stem)}"
        f"__run{int(run_id):03d}"
        f"__{_sanitize_id_component(vin)}"
        f"__t{int(traj_seq):04d}"
    )


def attach_trajectory_ids(df_run: pd.DataFrame, trajectories: list, source_csv: str, run_id: int):
    """Anexa trajectory_id no DataFrame e no JSON para permitir linkage."""
    if df_run is not None and not df_run.empty:
        trajectory_ids = []
        for seq, (_, row) in enumerate(df_run.iterrows(), start=1):
            vin = row.get('vin', 'unknown')
            trajectory_ids.append(build_trajectory_id(source_csv, run_id, vin, seq))
        df_run['trajectory_id'] = trajectory_ids

    if trajectories is None:
        return

    for seq, traj in enumerate(trajectories, start=1):
        vin = traj.get('vin', 'unknown')
        traj['trajectory_id'] = build_trajectory_id(source_csv, run_id, vin, seq)


def process_sumo_csv(
    input_csv: str,
    consumo_fabricante: float = CONSUMO_FABRICANTE,
    row_step: int = ROW_STEP,
    max_radius_km: float = MAX_RADIUS_KM,
    target_vehicle_id: str = TARGET_VEHICLE_ID,
) -> pd.DataFrame:
    """
    Processa CSV SUMO para veh0 com offset aleatório dentro do raio máximo.
    ...
    Args:
        ...
        max_radius_km: Raio máximo do offset aleatório em km

    Returns:
        Tupla (DataFrame com viagens agregadas, lista de trajetos)
    """
    print("="*70)
    print("🚗 PROCESSAMENTO SUMO → E1 REGISTRY (OFFSET ALEATÓRIO - veh0)")
    print("="*70)
    print(f"📄 Entrada: {input_csv}")
    print(f"🏭 Consumo fabricante: {consumo_fabricante} km/l")
    print(f"💰 Preço carbono: R$ {CARBON_PRICE}/ton")
    print(f"⭕ Raio máximo do offset aleatório: {max_radius_km} km")

    # Gerar offset aleatório para esta execução
    offset_x, offset_y, offset_dist_km, offset_angle_deg = generate_random_offset(max_radius_km)
    print(f"🎲 Offset gerado aleatoriamente:")
    print(f"   Ângulo: {offset_angle_deg:.1f}°")
    print(f"   Distância: {offset_dist_km:.4f} km")
    print(f"   offset_x (lat): {offset_x:.6f}°  |  offset_y (lon): {offset_y:.6f}°")
    ...
    # Preservar ordem original para desempate estável quando timestamps se repetem
    df['_row_order'] = range(len(df))
    
    print(f"   Veículos únicos: {df['vehicle_id'].nunique()}")

    available_vehicle_ids = [str(v) for v in df['vehicle_id'].dropna().unique().tolist()]
    if not available_vehicle_ids:
        print("⚠️  Nenhum vehicle_id encontrado no CSV.")
        return pd.DataFrame(), []

    if target_vehicle_id is not None:
        if target_vehicle_id in available_vehicle_ids:
            vehicle_to_process = target_vehicle_id
        else:
            vehicle_to_process = available_vehicle_ids[0]
            print(
                f"⚠️  vehicle_id '{target_vehicle_id}' não encontrado. "
                f"Usando '{vehicle_to_process}'."
            )
    else:
        vehicle_to_process = 'veh0' if 'veh0' in available_vehicle_ids else available_vehicle_ids[0]

    print(f"   🚙 Vehicle selecionado: {vehicle_to_process}")
    ...
    for vehicle_id, group in df.groupby('vehicle_id'):
        ...
        if str(vehicle_id) != vehicle_to_process:
            continue

        map_matching_attempts_vehicle = 0
        discarded_points_vehicle = 0

        # Ordenação estável para evitar zig-zag quando há timestamps iguais
        group = group.sort_values(by=['start_time', 'end_time', '_row_order'], kind='mergesort')
        ...
        # Distâncias em km (SUMO acumulado): usar max() numérico com fallback seguro
        total_distance_series = pd.to_numeric(group['distance'], errors='coerce')
        city_distance_series = pd.to_numeric(group['distance_city'], errors='coerce')
        highway_distance_series = pd.to_numeric(group['distance_highway'], errors='coerce')

        total_distance_km = float(total_distance_series.max()) if total_distance_series.notna().any() else 0.0
        distance_city_km = float(city_distance_series.max()) if city_distance_series.notna().any() else 0.0
        distance_highway_km = float(highway_distance_series.max()) if highway_distance_series.notna().any() else 0.0
        ...
        if ENABLE_MAP_MATCHING and MAP_MATCHING_AVAILABLE:
            ...
            if G_start is not None:
                map_matching_attempts_vehicle += 1
        ...
        if ENABLE_MAP_MATCHING and MAP_MATCHING_AVAILABLE:
            ...
            if G_traj is not None:
                ...
                for i, point in enumerate(trajectory_points_priv):
                    ...
                    # Tentar encontrar um ponto único com limite configurável de tentativas
                    ...
                    # Adicionar o ponto único encontrado
                    final_points.append(current_point)
        ...
        trajectory_points_diff_percent = (
            ...
            if trajectory_points_orig_count > 0 else np.nan
        )

        points_for_ratio = len(trajectory_points_orig)
        discard_ratio = (discarded_points_vehicle / points_for_ratio) if points_for_ratio > 0 else 0.0
        offset_cancelled = points_for_ratio > 0 and discard_ratio >= CANCEL_OFFSET_DISCARD_RATIO

        if offset_cancelled:
            cancellation_reason = (
                f"discard_ratio={discard_ratio:.2%} >= {CANCEL_OFFSET_DISCARD_RATIO:.0%} "
                f"(descartados={discarded_points_vehicle}, total={points_for_ratio})"
            )
            print(f"\n❌ OFFSET CANCELADO para {vin}: {cancellation_reason}")
            ...
            results.append({
                ...
                'start_lat_private': np.nan,
                'start_lon_private': np.nan,
                'end_lat_private': np.nan,
                'end_lon_private': np.nan,
                'start_map_matched': False,
                'end_map_matched': False,
                'pseudonimo': pseudonimo,
                'num_segments': len(group),
                'trajectory_points_orig_count': trajectory_points_orig_count,
                'trajectory_points_priv_count': trajectory_points_priv_count,
                'trajectory_points_diff': trajectory_points_diff,
                'trajectory_points_diff_percent': trajectory_points_diff_percent,
                'trajectory_distance_orig_km': np.nan,
                'trajectory_distance_priv_km': np.nan,
                'trajectory_distance_diff_km': np.nan,
                'map_matching_attempts': map_matching_attempts_vehicle,
                'discarded_points_map_matching': discarded_points_vehicle,
                'map_matching_points_total': points_for_ratio,
                'discarded_points_ratio': discard_ratio,
        ...
        trajectories.append({
            ...
            'num_points_original': trajectory_points_orig_count,
            'num_points_private': trajectory_points_priv_count,
            'num_points_diff': trajectory_points_diff,
            'num_points_diff_percent': trajectory_points_diff_percent,
        ...
        print(f"   🧭 Tentativas map matching: {map_matching_attempts_vehicle}")
        if discarded_points_vehicle > 0:
            print(f"   🗑️  Pontos descartados por limite de tentativas: {discarded_points_vehicle}")
    ...
    if df_result.empty or 'total_distance_km' not in df_result.columns:
        print("Viagens processadas: 0")
        print("⚠️  Nenhum registro processado para o vehicle selecionado.")
        print("="*70)
...
def save_trajectories_json(trajectories: list, output_dir: str):
    """Salva um JSON por trajeto em pasta dedicada e gera índice."""
    os.makedirs(output_dir, exist_ok=True)

    index_entries = []
    for i, traj in enumerate(trajectories, start=1):
        trajectory_id = traj.get('trajectory_id') or f"trajectory_{i:04d}"
        file_name = f"{trajectory_id}.json"
    ...
    print(f"💾 Trajetos individuais salvos em: {output_dir}")
    print(f"💾 Índice de trajetos salvo em: {index_path}")
...
def save_distance_analysis_csv(df: pd.DataFrame, output_csv: str):
    ...
    analysis_df = df[[
        ...
        'model',
        'total_distance_km',
        'trajectory_points_orig_count',
        'trajectory_points_priv_count',
        'trajectory_points_diff',
        'trajectory_points_diff_percent',
        'trajectory_distance_orig_km',
        'trajectory_distance_priv_km',
        'trajectory_distance_diff_km',
        'offset_cancelled',
        'map_matching_attempts',
        'discarded_points_map_matching',
        'start_lat_orig',
        'start_lon_orig',
        'end_lat_orig',
        'end_lon_orig',
    ...
    analysis_df.columns = [
        ...
        'Modelo',
        'Distancia_SUMO_km',
        'Num_Pontos_Trajeto_Original',
        'Num_Pontos_Trajeto_com_Offset',
        'Diferenca_Num_Pontos',
        'Diferenca_Num_Pontos_Percentual',
        'Distancia_Trajeto_Original_km',
        'Distancia_Trajeto_com_Offset_km',
        'Diferenca_Distancia_km',
        'Offset_Cancelado',
        'Tentativas_MapMatching',
        'Pontos_Descartados_MapMatching',
        'Start_Lat_Original',
        'Start_Lon_Original',
        'End_Lat_Original',
        'End_Lon_Original',
...
def process_single_file_all_runs_task(task: tuple) -> tuple:
    ...
    return input_file, worker_name, df_file_all, file_trajectories
...
def run_batch_parallel(
    ...
    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="csv-worker") as executor:
        future_to_task = {
    ...
    if all_dfs:
        ...
        trajectories_dir = os.path.join(output_dir, "all_runs_trajectories")
        save_trajectories_json(all_trajectories, trajectories_dir)
...
def main():
    ...
    for run_id in range(1, num_runs + 1):
        print(f"\n{'='*70}")
        print(f"▶  EXECUÇÃO {run_id}/{num_runs}")
        print(f"{'='*70}")
        ...
        df_trips.insert(0, 'run_id', run_id)
        df_trips.insert(1, 'source_csv', os.path.basename(input_file))
        attach_trajectory_ids(df_trips, trajectories, os.path.basename(input_file), run_id)
        all_dfs.append(df_trips)

        for t in trajectories:
            t['run_id'] = run_id
            t['source_csv'] = os.path.basename(input_file)
        all_trajectories.extend(trajectories)
    ...
    print(f"\n{'='*70}")
    print(f"✅ {num_runs} execuções concluídas — {len(df_all)} registros totais")
    print(f"{'='*70}")

    # Salvar CSV consolidado
    save_to_csv(df_all, output_file)

    # Salvar um JSON por trajeto em pasta dedicada
    trajectories_dir = output_file.replace('.csv', '_trajectories')
    save_trajectories_json(all_trajectories, trajectories_dir)

    # Salvar CSV de análise de distâncias consolidado
    analysis_file = output_file.replace('.csv', '_distance_analysis.csv')
    save_distance_analysis_csv(df_all, analysis_file)
