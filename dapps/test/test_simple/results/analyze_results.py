...
def calculate_aggregate_stats(results: dict) -> pd.DataFrame:
    ...
    for num_workers, df in sorted(results.items()):
        ...
        # Throughput CORRETO: total_successful / tempo_do_teste
        # Usamos a duração máxima (worker mais lento) como aproximação do tempo total
        max_duration = df['duration_s'].max()
        avg_duration = df['duration_s'].mean()
        
        # Throughput real baseado no tempo total do teste
        real_throughput = total_successful / max_duration if max_duration > 0 else 0
        
        # Throughput "teórico" (soma dos individuais - INCORRETO mas mantemos para comparação)
        sum_throughput = df['throughput_tx_s'].sum()
        
        # Eficiência: quantos workers realmente completaram
        workers_completed = len(df[df['total_txs'] > 0])
        ...
        p99_latency = df['avg_latency_ms'].quantile(0.99)
        
        min_latency_global = df['min_latency_ms'].min()
        max_latency_global = df['max_latency_ms'].max()
        ...
        stats = {
            ...
            'throughput_real_tx_s': round(real_throughput, 2),
            'throughput_sum_tx_s': round(sum_throughput, 2),
...
def print_summary_table(df_stats: pd.DataFrame):
    ...
    for _, row in df_stats.iterrows():
        print(f"{row['num_workers']:<10} "
              ...
              f"{row['throughput_real_tx_s']:<15.2f} "
    ...
    best_throughput = df_stats.loc[df_stats['throughput_real_tx_s'].idxmax()]
    print(f"✓ Melhor throughput: {best_throughput['num_workers']} workers "
          f"({best_throughput['throughput_real_tx_s']:.2f} tx/s)")
