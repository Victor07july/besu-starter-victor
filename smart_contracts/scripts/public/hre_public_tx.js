...
const hosthost = besu.rpcnode.url;
...
async function main() {
  ...
  const fetchRequest = new FetchRequest(besu.rpcnode.url);
  fetchRequest.setHeader("Authorization", `Bearer ${accessToken}`);


  const provider = new ethers.JsonRpcProvider(fetchRequest, 1337);


  const wallet = new ethers.Wallet(accountPrivateKey, provider);

  createContract(provider, wallet, contractAbi, contractBytecode, 0)
